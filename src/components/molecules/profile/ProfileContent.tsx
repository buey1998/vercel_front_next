...
!bg-neutral-800 text-xs uppercase hover:border-secondary-main"
        ...
        <div className="login-telegram mt-8 w-full md:mt-0 md:w-[98%] lg:w-[90%]">
          ...
          >
            ...
            <div
              className="flex h-[30px] w-[120px] items-center justify-center rounded-[10px] bg-secondary-main"
              id="login"
            >
              <Link href="https://discord.com/api/oauth2/authorize?client_id=1123574269398499448&redirect_uri=http%3A%2F%2Flocalhost%3A4004%2Fauth%2Fcallback%2Fdiscord&response_type=code&scope=identify%20email">
                Discord Login
              </Link>
            </div>
            {/* {!session && ( */}
  ...
  window.onload = async() => {
    ...
    let [accessToken, tokenType, state, code] = [
    ...
    if (code) {
        ...
        "https://discord.com/api/oauth2/token",
        {
          ...
          body: new URLSearchParams({
            client_id: '1116321695117103118',
            client_secret: 'DNYTCbRgOZBVhfyfbgjO9O_I2elvyXod',
            code,
    ...
    if (
      localStorage.getItem("oauth-state") !== state
    ) {
    ...
    const res = await fetch("https://discord.com/api/users/@me", {
      ...
      .then((response) => {
        ...
        document.getElementById(
          "info"
        ).innerHTML += ` ${username}#${discriminator}`;
      })
      .catch((err) => {
        console.log(err);
      });
